...
IF [%1]==[] ( 
	SET RUNCMD=matlab -nosplash -nodesktop -logfile %logfile% -r "restoredefaultpath(); runCore(); exit" /minimize
	) ELSE IF [%1]==[/DEBUG] ( 
	SET RUNCMD=matlab -r "dbstop if error; runCore" 
	) ELSE ( SET RUNCMD=matlab -nosplash -nodesktop -logfile %logfile% -r "restoredefaultpath(); runCore('%~1'); exit" /minimize)
